...
from utils import get_nan_rows, complete_df_random, complete_df_mode
from learn_structure import hill_climbing

# Hill climbing algorithm for ets
#


def sem_tractable(data_frame, et0=None, u=5, metric='bic', metric_params=None, chc=False, tw_bound_type='b', tw_bound=5, optimization_type='size', k_complex=0.1, cores=multiprocessing.cpu_count(), forbidden_parent=None, complete_prior = 'mode', add_only = False, custom_classes=None):
    """Hard tractable SEM (TSEM-hard) 
    ...
    if custom_classes is None:
        data_missing = data_type.data(data_frame)
    else:
        data_missing = data_type.data(data_frame, classes= custom_classes)
    ...
    for i in range(0, len_nodes):
        ...
        score_best.append(score_function(data_complete, i, parents, metric, metric_params))
    ...
    while ok_to_proceed:
        ...
        ta = time()
        et_new, score_new, time_score, time_compile, time_opt, best_change, forbidden_parents = best_pred_hc(data_complete, et, score_best, forbidden_parents, u, metric, metric_params, chc, tw_bound_type, tw_bound, optimization_type, k_complex, cores, add_only)
        ...
        if sum(score_new) > sum(score_best):
            ...
            fiti.learn_parameters(data_complete, alpha = 0.01)
            fiti.mpe_data(data_complete, data_missing, nan_rows)
...
def best_pred_hc(data, et, score, forbidden_parents, u=5, metric='bic', metric_params=None, chc=False, tw_bound_type='b', tw_bound=5, optimization_type='size', k_complex=0.1, cores=multiprocessing.cpu_count(), add_only = False):
    change_lst = []
    ...
    best_result = local_changes_par(min(cores, len(change_lst)), change_lst, data, et, score, forbidden_parents, metric, metric_params, chc, tw_bound_type, tw_bound, optimization_type, k_complex)
